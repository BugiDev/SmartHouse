...
public class MainDevicesListActivity extends ListActivity {
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		if (item.equalsIgnoreCase("Lights")) {
			...
			if (airconditionList.size() > 1) {
				...
				intent.putExtra("temperature", airconditionList.get(0)
						.getTemperature());
			...
			if (windowBlindsList.size() > 1) {
				...
				windowBlindsTask = new ChangeWindowBlindsDirectionTask(this,
						windowBlindsList.get(0).getId(), windowBlindsList
								.get(0).getRoom(), adapter, position);
			...
			turnAlarmOnOff = new TurnOnOffAlarmTask(onOff, cameraList.get(0).getPath(), position, adapter, this);
	...
	private void getDevicesFromJSON() {
		...
		try {
			...
			for (int i = 0; i < jArray.length(); i++) {
				...
				Camera camera_single = new Camera(json_data.getInt("id"),
	...
	public void initializeList() {
		...
		allDevices.add("Temperature: ".concat(
				Integer.toString(temperatureList.get(0).getTemperature()))
				.concat("°C"));
	...
	public void prepDialog() {
		...
		TextView text = (TextView) dialog
				.findViewById(R.id.dialog_temperature_text);
		text.setText("Temperature: ".concat(
				Integer.toString(temperatureList.get(0).getTemperature()))
				.concat("°C"));

		ImageView image = (ImageView) dialog
				.findViewById(R.id.dialog_temperature_image);
